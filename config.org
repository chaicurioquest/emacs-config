...
(setq default-directory
      (cond ((eq my-device 'laptop) (expand-file-name "wspace/org/notes/" (getenv "HOME")))
            ((eq my-device 'termux) (expand-file-name "storage/shared/notes/" (getenv "HOME")))
            ((eq my-device 'tablet) (expand-file-name "path/to/tablet/notes/" (getenv "HOME")))
            (t (getenv "HOME"))))
...
(use-package org
  ...
  ;; Optional: fallback export processor
  (setq org-cite-export-processors
        '((latex biblatex "backend=biber,style=authoryear")))

  ;; Use latexmk with lualatex, biber, and glossaries, inside build/
(setq org-latex-pdf-process
      '("mkdir -p build"  ;; Create org/notes/build/ if it doesn't exist
        "mv %f build/"   ;; Move tex file to org/notes/build/
        "latexmk -lualatex -r ../setup/.latexmkrc -outdir=build -f build/%b.tex >> build/latex.log 2>&1"  ;; Run latexmk from org/notes/, using .latexmkrc from org/setup/
        ))

;; Overall notes: This list defines the shell commands Org mode runs sequentially for LaTeX-to-PDF export.
;; It handles directory management, compilation with lualatex, bibliography (biber), glossaries, and cleanup.
;; Customize paths if your .bib or glossary files are elsewhere. If errors occur, check 'build/latex.log' for details.

  ;; Avoid \usepackage[utf8]{inputenc} for LuaLaTeX
  (setq org-latex-inputenc-alist '(("utf8" . nil)))

  ;; Tell LaTeX to search for files in ../include
  (setenv "TEXINPUTS" "../include:")

  ;; Optional: warn if setup-latex.org is missing
  (let ((setup-file (expand-file-name "../include/setup-latex.org" default-directory)))
...
(use-package yankpad
  ...
  (setq yankpad-file
        (cond
         ((eq my-device 'laptop) (expand-file-name "org/yankpad.org" user-emacs-directory))
         ((eq my-device 'termux) (expand-file-name "org/yankpad.org" default-directory))
         ((eq my-device 'tablet) (expand-file-name "yankpad.org" default-directory))
...
(use-package citar
  ...
  (citar-bibliography (condition-case nil
                          (list (expand-file-name "include/references.bib" default-directory))
...
(add-hook 'before-save-hook
  (lambda ()
    (when (derived-mode-p 'org-mode)
      (save-excursion
        ;; Remove existing LAST_MODIFIED line
        (goto-char (point-min))
        ...
        (let ((metadata-end (point)))
          ...
          ;; Insert a newline only if not already there
          (unless (looking-back "\n" nil)
            (insert "\n"))
          ;; Insert LAST_MODIFIED with correct timestamp
