...
(use-package citar
  :straight t
  :custom
  (citar-bibliography (list (expand-file-name "references.bib" default-directory)))  ;; Path to your BibTeX file
  (citar-notes-paths (list default-directory))  ;; Where to save notes (e.g., Org files)
  (citar-library-paths (list (expand-file-name "pdfs" default-directory)))  ;; Zotero PDF storage dir
  (citar-open-note-function 'orb-citar-edit-note)  ;; Use org-roam-bibtex for notes
  :bind ("C-c r c" . citar-open))  ;; Key to open Bib entry/PDF
#+END_SRC
