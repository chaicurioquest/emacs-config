...
(add-hook 'emacs-startup-hook
          (lambda ()
            ...
            (message "Emacs ready in %.2fs | GC runs: %d"
...
;; Redirect Custom writes away from tangle-generated config.el
...
(defvar setupfile (expand-file-name "latex/setup-latex.org" my/notes-root-dir))
...
plus recursive .org files from the 'roam' subdirectory, excluding unwanted dirs."
  ...
  (let* ((notes-dir (file-name-as-directory my/notes-root-dir))
         ...
         (excluded-dirs '("build" "ltximg" "images" ".attachments" ".autosaves" ".backups" "bib" "latex" ".git"))
...
\\documentclass[presentation,aspectratio=169,12pt]{beamer}
\\usepackage{amsmath}
\\usepackage{graphicx}
\\usepackage{hyperref}"
      ("\\section{%s}" . "\\section{%s}")
      ("\\begin{frame}[t,fragile]{%s}" . "\\end{frame}")
...
;; Flyspell + Hunspell (en_GB)
;; ispell settings go in :init — must exist before flyspell loads
;; so the first hook invocation already picks up the right backend.
#+BEGIN_SRC emacs-lisp
(use-package flyspell
  :straight nil  ; built-in with Emacs distribution
  :hook
  ;; Hooks registered by use-package at top-level — no deferral race.
  ((text-mode . flyspell-mode)       ; full spell-check for prose/org
  (prog-mode . flyspell-prog-mode))  ; strings & comments only for code
  ...
  :config
  ;; Suppress "Invalid search bound" error on C-g inside Org buffers.
  (advice-add 'org-mode-flyspell-verify :around
              (lambda (orig &rest args)
                (ignore-errors (apply orig args)))))
  ;; Correction interface for M-$ (popup candidate list).
...
(use-package auctex
  ...
  :hook ((LaTeX-mode . turn-on-reftex)
       (LaTeX-mode . flyspell-mode))  ;; Optional: RefTeX for refs, spell-check
  :config
...
(use-package recentf
  ...
  (recentf-exclude
  (list (expand-file-name "roam/" my/notes-root-dir)
...
#+BEGIN_SRC emacs-lisp
;; Ensure Magit's deps are straight-managed before Magit loads
(require 'transient)
...
;; Remove stale hook at TOP LEVEL — must run before any magit command fires.
;; :config runs too late (only when Magit loads, after the hook already fired).
(with-eval-after-load 'magit
  (remove-hook 'magit-post-refresh-hook #'magit-run-post-commit-hook))
#+END_SRC
...
;; Wrap text only in Org/text; keep code buffers truncated (no soft-wrap).
...
(defun magit-pull-org-repo ()
  "Pull the Org notes repo from upstream."
  (interactive)
  (let ((default-directory "/wspace/org/"))
    (magit-pull-from-upstream nil)))

(defun magit-push-org-repo ()
  "Stage all org files (tracked + new), commit with message prompt, then push."
  (interactive)
  (let ((default-directory "/wspace/org/"))
    ;; Save any unsaved Org buffers first
    (org-save-all-org-buffers)
    ;; Stage tracked modified files
    (magit-stage-modified)
    ;; Stage any new untracked .org files
    (dolist (file (magit-untracked-files))
      (when (string-match-p "\\.org\\'" file)
        (magit-stage-file file)))
    ;; Commit synchronously if anything staged
    (if (magit-anything-staged-p)
        (let ((msg (read-string "Commit message: " "update")))
