...
;; Optional: VLSI-specific configs for TikZ/CircuitTikZ diagrams
;; Uncomment to enable (requires TeX Live with tikz/circuitikz, ImageMagick, Ghostscript, ditaa.jar)
;; (my/tangle-if-needed "org/customxtn.org" "org/customxtn.el")
;; (load (expand-file-name "org/customxtn.el" user-emacs-directory) nil 'nomessage)
#+END_SRC
...
(defun my--run-shell-command-async (cmd message-prefix)
  ...
  (let ((buf (get-buffer-create "*Git Output*")))
    (with-current-buffer buf
      (read-only-mode -1)                     ;; make buffer editable
      (erase-buffer)                          ;; clear old contents
      (insert (format "%s: Running %s\n\n"    ;; header line
                      message-prefix cmd)))
    (async-shell-command cmd buf)))           ;; run command asynchronously, send output here
...
(defun git-push-repo ()
  "Stage all changes, commit if needed, then push asynchronously."
  (interactive)
  (let ((repo-dir (file-truename default-directory)))
    (my--run-shell-command-async
     (format
      "cd %s && git add -A && (git diff --cached --quiet || git commit -m 'Update from laptop') && git push origin main"
