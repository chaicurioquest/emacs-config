...
(defvar bib-path
  (expand-file-name "bib/references.bib" (file-name-directory default-directory))
   "Absolute path to BibTeX file")

(defvar my-citar-library-paths
  (list "/wspace/src/zotero-kbase/storage")
  "List of absolute paths to Citar libraries (e.g., Zotero storage).")
...
** Org Roam and Org Roam path definition
#+BEGIN_SRC emacs-lisp
...
#+BEGIN_SRC emacs-lisp
(defvar my/org-notes-dir nil "The root directory for org notes.")

(defun my/update-agenda-files ()
  "Set `org-agenda-files` to top-level .org files in `my/org-notes-dir` or `default-directory`,
plus recursive .org files from the 'roam' subdirectory, excluding unwanted dirs and non-regular files."
  (interactive)
  (let* ((notes-dir (if my/org-notes-dir
                        my/org-notes-dir
    ...
    (unless my/org-notes-dir
      (setq my/org-notes-dir notes-dir))))
...
(add-hook 'after-save-hook
          (lambda ()
            (when (and (derived-mode-p 'org-mode)
                       buffer-file-name
                       my/org-notes-dir  ; Ensure it's set
                       (file-in-directory-p buffer-file-name my/org-notes-dir))
              (my/update-agenda-files))))
...
(use-package pdf-tools
  ...
  :defer t
  :config
  ;; Use existing epdfinfo binary in Termux (if applicable)
  ...
  (message "✅ PDF-tools ready for viewing."))
#+END_SRC

** pdf-view-mode
#+BEGIN_SRC emacs-lisp
;; === PDF Tools & Viewing Setup ===
(require 'pdf-tools)
(pdf-tools-install)  ;; Compile and setup pdf-tools
...
#+BEGIN_SRC emacs-lisp
;; Citar (core citation management)
(use-package citar
  :straight t
  :commands (citar-insert-citation citar-create-note)
  :custom
  (citar-bibliography (list bib-path))
  (citar-library-paths my-citar-library-paths)
  (citar-notes-paths (list (expand-file-name "literature" org-roam-directory)))  ;; Path to your bib notes dir
  (citar-file-note-extensions '("org"))  ;; Extensions for note files (e.g., .org for Org-roam)
  (org-cite-insert-processor 'citar)
  (org-cite-follow-processor 'citar)
  (org-cite-activate-processor 'citar)
  (citar-file-open-function 'find-file)  ; Open PDFs in Emacs
  :config
  (citar-capf-setup)
  (message "✅ Citar ready."))

  ;; Org-local binding (no global collision)
...
(defun my/citar-insert-or-create-note ()
  ...
  (if (derived-mode-p 'org-mode)
      (call-interactively 'citar-insert-citation)  ;; Default behavior in Org
      (call-interactively 'citar-create-note)))  ;; Create note globally otherwise
...
;; Optional: General LaTeX export configs (requires TeX Live, latexmk)
...
;; Optional: Engineering-specific configs for TikZ/CircuitTikZ diagrams
(my/tangle-if-needed "org/customxtn.org" "org/customxtn.el")
(load (expand-file-name "org/customxtn.el" user-emacs-directory) nil 'nomessage)
