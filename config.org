...
(use-package citar
  ...
  (citar-bibliography (list (expand-file-name "references.bib" default-directory)))
...
(add-hook 'before-save-hook
          (lambda ()
            (when (derived-mode-p 'org-mode)
              ;; Update or insert #+LAST_MODIFIED:
              (save-excursion
                (goto-char (point-min))
                (if (re-search-forward "^#+LAST_MODIFIED: .*$" nil t)
                    (replace-match (format "#+LAST_MODIFIED: %U"))
                  (goto-char (point-min))
                  (if (re-search-forward "^$" nil t)
                      (insert "#+LAST_MODIFIED: %U\n")
                    (goto-char (point-max))
                    (insert "\n#+LAST_MODIFIED: %U\n")))))))
#+END_SRC
...
;; Add keybinding for Org capture (necessary to trigger templates)
