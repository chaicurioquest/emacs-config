...
(use-package org
  ...
  (setq org-latex-pdf-process
        '("mkdir -p build" ;; Create build/ directory in org/notes/ if it doesn't exist
          "mv %f build/" ;; Move generated .tex file to org/notes/build/
          "rm -f build/latex.log" ;; Remove previous build/latex.log to start fresh
          "latexmk -lualatex -r ../setup/.latexmkrc -outdir=build -f build/%b.tex > build/latex.log 2>&1" ;; Run latexmk with lualatex, using .latexmkrc from org/setup/, output to build/, and redirect logs
          "mv build/%b.pdf ./" ;; Move generated PDF to org/notes/
          ))
  ;; Suppress only "Shell command succeeded with no output" messages during export
  (advice-add 'message :around
              (lambda (orig-fun format-string &rest args)
                (unless (string-match-p "Shell command succeeded with no output" format-string)
                  (apply orig-fun format-string args))))
  ;; Prevent Org from adding \usepackage[utf8]{inputenc} (incompatible with lualatex)
  (setq org-latex-inputenc-alist '(("utf8" . nil)))
  ;; Set TEXINPUTS to include org/setup/ for LaTeX input files (e.g., references.bib, acronyms)
  (setenv "TEXINPUTS" (concat "../setup:" (getenv "TEXINPUTS")))
  ;; Warn if setup-latex.org is missing in org/setup/
