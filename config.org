...
(when (eq my-device 'laptop) ; Skip heavy PDF on mobile
  (use-package pdf-tools
    ...
    ;; Automatically revert unmodified PDF buffers without asking when the file changes
...
(when (eq my-device 'laptop)
  (use-package org-pdftools
    ...
    ;; Add behavior for jumping to notes
    (condition-case nil
        (add-to-list 'org-noter-notes-window-behavior '(org-noter-pdftools-jump-to-note))
      (error (message "Org-noter-pdftools integration failed")))
    ;; Optional: Customize icons/colors for annotations (from org-noter-pdftools)
    (setq org-noter-pdftools-free-pointer-icon "PencilTip"
          org-noter-pdftools-free-pointer-color "DodgerBlue"
          org-noter-pdftools-markup-pointer-color "DarkOrange")))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
;; Unfill package: M-Q removes line breaks from PDF text (inverse of M-q fill-paragraph)
(use-package unfill
  :straight t
...
;; org-glossary--register-buffer-dependencies has no cycle detection
;; (unlike org-glossary--get-terms which does). This adds one.
