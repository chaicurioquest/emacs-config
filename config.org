...
| C-s           | consult-line                          | Search lines                                         | Global                   |
...
(use-package pdf-tools
  ...
  :init
  (unless (eq my-device 'laptop)
    (setq pdf-tools-enabled-modes nil)) ; Disable elsewhere
  :config
  (setq pdf-info-epdfinfo-program
        (expand-file-name "build/server/epdfinfo" (straight--build-dir "pdf-tools")))
  ;; Run installation/build once at startup
  ...
  ;; Pre-clear PDF cache on view start
  (add-hook 'pdf-view-mode-hook (lambda () (pdf-cache-clear-data (buffer-file-name))))
...
(use-package org-noter-pdftools
  :straight t  ; Install from MELPA
  :defer t
  :after (org-noter pdf-tools)
  :commands org-noter-pdftools-jump-to-note
  :config
  ;; Add this guard: Only load if no errors
  (condition-case err
      (progn
        (add-to-list 'org-noter-notes-window-behavior '(org-noter-pdftools-jump-to-note))
        (message "org-noter-pdftools loaded successfully"))
    (error (message "org-noter-pdftools load failed: %S. Disabling." err))))
#+END_SRC
