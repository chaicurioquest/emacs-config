...
(use-package org
  ...
  ;; Default to biblatex with authoryear for LaTeX exports (fallback if no #+CITE_EXPORT)
  (setq org-cite-export-processors '((latex biblatex "backend=biber,style=authoryear")))
  ;; Global bib file with error checking (relative to default-directory)
  (setq org-cite-global-bibliography
        (let ((bib-path (expand-file-name "include/references.bib" default-directory)))
          (if (file-exists-p bib-path)
              (list bib-path)
            (message "⚠️ Bibliography file %s not found" bib-path)
            nil)))
  ;; LaTeX export process with latexmk
  (setq org-latex-pdf-process '("latexmk -lualatex -output-directory %o %f"))
  ;; Org-mode display settings
  (setq org-startup-indented t
        org-hide-emphasis-markers t)
  ;; Enable citar completion-at-point
  (with-eval-after-load 'citar
    (citar-capf-setup))
  ;; Check setup-latex.org existence (relative to user-emacs-directory)
  (let ((setup-file (expand-file-name "setup/setup-latex.org" user-emacs-directory)))
    (unless (file-exists-p setup-file)
      (message "⚠️ Setup file %s not found" setup-file))))
#+END_SRC
...
(defvar my/generic-note-template
  (with-temp-buffer
    ...
    (while (search-forward "${setupfile}" nil t)
      (replace-match (expand-file-name "setup/setup-latex.org" user-emacs-directory) t t))
    (buffer-string)))
