...
** Citar + Org noter + pdf annoation on roam bib file
#+BEGIN_SRC emacs-lisp
;; Required packages
(require 'citar)
(require 'org-noter)
...
#+BEGIN_SRC emacs-lisp
;; --- Load template file content helper ---
(defun my/load-template (filename)
  "Load template FILENAME from user-emacs-directory/template."
  (let ((template-path (expand-file-name filename (expand-file-name "template" user-emacs-directory))))
    (if (file-exists-p template-path)
        (with-temp-buffer
          ...
          (buffer-string))
      (error "Template file not found: %s" template-path))))
#+END_SRC

#+BEGIN_SRC emacs-lisp
;; --- Dynamic roam-bib.org capture template generator ---
(defun my/load-roam-bib-template ()
  "Load roam-bib.org template and inject citation metadata."
  (let* ((template (my/load-template "roam-bib.org"))
         (citekey (org-roam-capture-get :citar-citekey))
         (entry (citar-get-entry citekey))
         (title (or (cdr (assoc "title" entry)) (org-roam-capture-get :title) ""))
         (doi (or (cdr (assoc "DOI" entry)) ""))
         (date (or (cdr (assoc "date" entry)) ""))
         (year (or (cdr (assoc "year" entry)) (and (> (length date) 3) (substring date 0 4)) ""))
         (file (or (car (ignore-errors (citar-get-files citekey))) ""))
         (noter-document (if (string-empty-p file) "%^{Noter Document Path}" file)))
    (format template title citekey doi year noter-document citekey)))
#+END_SRC

#+BEGIN_SRC emacs-lisp
;; ;; Add bibliography capture template deferred until org-roam loads
(with-eval-after-load 'org-roam
  (setq org-roam-capture-templates
        (append
         (cl-remove-if (lambda (tpl) (string= (car tpl) "b")) org-roam-capture-templates)
         `(("b" "bibliography note" plain
            (function my/load-roam-bib-template)
            :target (file+head "literature/${citar-citekey}.org"
                              ,(my/load-template "roam-bib.org"))
            :unnarrowed t)))))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
;; -----------------------------
;; Helper: Extract first string path from complex data structures
;; -----------------------------
...
(defun my/open-paper-and-note ()
  "Open PDF and roam note side-by-side for citation key."
  (interactive)
  (let* ((key (citar-select-ref)))
    (unless key (user-error "No citation key selected"))
    (setq my/last-citar-key key)  ;; <-- This is important to track current citation key
    (let* ((entry (citar-get-entry key))
           (files (ignore-errors (citar-get-files key)))
           (pdf (let ((p (my/citar-extract-first-path files))) (and p (file-exists-p p) (expand-file-name p))))
           (title (or (cdr (assoc "title" entry)) key))
           (note-file (expand-file-name (concat key ".org") (expand-file-name "literature" org-roam-directory))))
      (unless (file-directory-p (file-name-directory note-file)) (make-directory (file-name-directory note-file) t))
      (delete-other-windows)
      (find-file note-file)
      (let ((left-win (selected-window)))
        (split-window-right)
        (let ((right-win (next-window left-win)))
          (select-window right-win)
          (if pdf
              (progn
                (let ((display-buffer-alist nil)) (find-file pdf))
                (when (fboundp 'pdf-view-fit-page-to-window) (pdf-view-fit-page-to-window)))
            (switch-to-buffer "*scratch*")
            (message "[LOG] No PDF found for %s, showing scratch buffer." key))
          (select-window left-win)
          ;; Create note template if note doesn't exist
          (unless (file-exists-p note-file)
            (let ((id (org-id-new)))
              (insert (format "#+TITLE: %s\n#+ROAM_REFS: cite:%s\n:PROPERTIES:\n:ID: %s\n:NOTER_DOCUMENT: %s\n:END:\n\n* Overview\n\n* Annotations\n\n* Bib Entry\n[cite:@%s]\n"
                              title key id (or pdf "") key))
              (save-buffer)))
          ;; Ensure NOTER_DOCUMENT property present
          (save-excursion
            (goto-char (point-min))
            (unless (re-search-forward ":NOTER_DOCUMENT:" nil t)
              (when pdf
                ...
                (save-buffer))))
          ;; Start org-noter session if available
          (when (and (featurep 'org-noter) pdf)
            (org-noter))
          (message "[LOG] Opened paper and note for %s" key))))))
...
(defun my/citar-find-note-file-for-key (key)
  "Return first org-roam note file matching KEY or nil."
...
(defun my/pdf-insert-selection-to-note ()
  ...
  (let* ((selection (ignore-errors (pdf-view-active-region-text)))
         (page (ignore-errors (when (fboundp 'pdf-view-current-page)
                                (pdf-view-current-page))))
         (time (format-time-string "%Y-%m-%d %H:%M"))
         (note-buf (my/citar-find-note-file-for-key my/last-citar-key)))
    (unless note-buf
      (user-error "Could not find bib note file. Run C-c i a first."))
    (with-current-buffer (find-file-noselect note-buf)
      (goto-char (point-min))
      ...
      (let ((heading (format "** Page %s — %s\n" (or page "-") time)))
        ...
        (when page
          ...
          (insert ":END:\n\n"))
        ;; Always insert something, show message if selection is missing
        (insert (format "%s\n\n" (or selection "[No selection captured]")))
        (save-buffer)))
