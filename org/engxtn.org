   ...
   (pdftoppm
    (setq entry
          '(:name "pdftoppm-fast"
            ...
            :image-converter ("pdftoppm" "-png" "-r" "300" "%f" "%O"))))

   ;; 2. Fallback: ImageMagick (convert/magick) + Ghostscript
   ((and magick gs)
    (setq entry
          `(:name "imagemagick"
            :programs (,engine ,(file-name-nondirectory magick))
            :description "ImageMagick fallback"
            :image-input-type "pdf"
            :image-output-type "png"
            :latex-compiler (,(my/org-latex-make-latex-cmd engine))
            :image-converter (,magick "-density" "300" "%f" "-trim" "-flatten" "%O")))))

  (when entry
    (setq org-preview-latex-default-process (if pdftoppm 'pdftoppm-fast 'imagemagick)
          org-preview-latex-process-alist (list entry))
    (message "LaTeX preview enabled: %s" (if pdftoppm "pdftoppm (fast)" "ImageMagick"))))
#+END_SRC
