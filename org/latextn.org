#+TITLE: General LaTeX Export Configuration
#+PROPERTY: header-args:emacs-lisp :tangle yes
#+DATE: [%<%Y-%m-%d>]

* LaTeX export & preview configuration
#+BEGIN_SRC emacs-lisp
;; ------------------------------
;; Latex Configuration for Emacs Org to Latex Export
;; Default engine: LuaLaTeX
;; pdfLaTeX kept as standby for compatibility
;; ------------------------------

;; Prefer LuaLaTeX for export & Babel execution
(setq org-latex-compiler "lualatex")  ;; Hardcode to avoid nil
;; (setq org-babel-latex-compiler "lualatex")  ;; Hardcode to avoid nil

;; ob-latex babel compilation -- never uses latexmk
  (setq org-babel-latex-pdf-process
      '("lualatex --shell-escape -interaction=nonstopmode -output-directory %o %f"))

;; Commented since setup latex file handle latex engine compatibility: Dated 2026-02-16
;; Remove inputenc and fontenc with exact match (fourth element is engine list)
;;(setq org-latex-default-packages-alist
;;      (delete '("AUTO" "inputenc" t ("pdflatex"))
;;              (delete '("T1" "fontenc" t ("pdflatex")) org-latex-default-packages-alist)))


;; ------------------------------
;; Tier 1: Essential Default Packages
;; ------------------------------
(setq org-latex-default-packages-alist
      '(("" "amsmath" t)           ; Math support
        ("" "graphicx" t)          ; Images - ESSENTIAL
        ("" "longtable" nil)       ; Multi-page tables
        ("normalem" "ulem" t)      ; Underline/strikethrough
        ("" "capt-of" nil)         ; Captions outside floats
        ("" "hyperref" nil)))      ; Links/PDF metadata - ESSENTIAL

;; Note: graphicx, rotating, hyperref loaded in setup-latex.org
;; Note: wrapfig removed (rarely used, avoid conflicts)

;; ------------------------------
;; Tier 1: Table Defaults
;; ------------------------------
;; Every Org table exports as longtable (page-breakable) with no
;; forced \begin{center} wrapper — avoids the tabular+center+landscape
;; page-explosion bug. Override per-table with #+ATTR_LATEX: :environment ...
(setq org-latex-default-table-environment "longtable")
(setq org-latex-tables-centered nil)
;; Enable booktabs for every table
(setq org-latex-tables-booktabs t)

;; ------------------------------
;; Preview Configuration
;; ------------------------------

;; Show previews only when you ask (change to t to enable automatically)
;; (setq org-startup-with-latex-preview t)
;; (setq org-preview-latex-default-process 'imagemagick)

;; Tweak preview scale and other options
;; (with-eval-after-load 'org
;;   (plist-put org-format-latex-options :scale 1.1)) ;; 1.0–1.3 typical

;; Tweak preview scale and other options
(with-eval-after-load 'org
  (plist-put org-format-latex-options :scale 1.6) ;; default 1.0
  (plist-put org-format-latex-options :foreground 'default)  ; ✏️ was "White"
  (plist-put org-format-latex-options :background "Transparent")  ; ✏️ was "Transparent"
  (setq org-highlight-latex-and-related '(native latex script))
  (setq org-startup-with-latex-preview nil))


;; Customize preview image directory to use temp dir
(setq org-preview-latex-image-directory (concat temporary-file-directory "ltximg/"))

;; Auto-toggle LaTeX fragment preview: renders on cursor-leave, un-renders on cursor-enter
(use-package org-fragtog
  :straight t
  :hook (org-mode . org-fragtog-mode))
  
;; ------------------------------
;; CDLaTeX Customizations
;; ------------------------------
(with-eval-after-load 'cdlatex
  (add-to-list 'cdlatex-command-alist
               '("df" "Insert dfrac" "\\dfrac{?}{}" cdlatex-position-cursor nil nil t))
  (add-to-list 'cdlatex-command-alist
               '("pd" "Partial derivative" "\\frac{\\partial ?}{\\partial }" cdlatex-position-cursor nil nil t)))

;; ------------------------------
;; Labels and References
;; ------------------------------

;; In cdlatex: Auto-generate labels for equations, figures, and tables without prompting (prompt only for sections)
;; (setq reftex-insert-label-flags '("s" "s")) remove this line if the following is working
(setq reftex-insert-label-flags '("sefhd" ""))

;; Prefer custom #+NAME labels in LaTeX export
(setq org-latex-prefer-user-labels t)

;; Custom function in native org reference. Auto-generate labels for equations, figures, and tables
(defun org-auto-label (prefix)
  "Generate a unique label with PREFIX and an incrementing number.
Scans the buffer for existing #+name: lines with the prefix and a numeric suffix."
  (save-excursion
    (goto-char (point-min))
    (let ((max-num 0))
      (while (re-search-forward (format "#\\+name:[ \t]*%s:\\([0-9]+\\)" (regexp-quote prefix)) nil t)
        (let ((num (string-to-number (match-string 1))))
          (when (> num max-num)
            (setq max-num num))))
      (format "%s:%d" prefix (1+ max-num)))))

;; ------------------------------
;; Environment Variables for LaTeX
;; ------------------------------

;; Set TEXINPUTS to include subdirs under root, guaranteeing default search path is preserved
(setenv "TEXINPUTS" (concat
                     (file-name-as-directory my/notes-root-dir) "latex:"
                     (file-name-as-directory my/notes-root-dir) "images:"
                     (file-name-as-directory my/notes-root-dir) "org/latex:"
                     (or (getenv "TEXINPUTS") "")
                     ":"))

;; Set BIBINPUTS to include bib/ under root, same trailing-colon guarantee
(setenv "BIBINPUTS" (concat
                     (file-name-as-directory my/notes-root-dir) "bib:"
                     (or (getenv "BIBINPUTS") "")
                     ":"))

;; ------------------------------
;; Export Process
;; ------------------------------
;; Define LaTeX-to-PDF export commands with explicit multiple runs, without separate build dir
(let ((latexmkrc (expand-file-name ".latexmkrc" (file-name-directory setupfile))))
  (setq org-latex-pdf-process
        `(,(format "timeout 300 latexmk -lualatex -r %s -output-directory=%%o %%f" latexmkrc))))

;; Keep .log for error inspection; remove other intermediates
(setq org-latex-logfiles-extensions
      '("aux" "blg" "fdb_latexmk" "fls" "out" "toc"))        

;; Use minted (Pygments) for syntax-highlighted source blocks in LaTeX/PDF export
(setq org-latex-listings 'minted)

;; minted language name overrides for LaTeX/PDF export
(with-eval-after-load 'ox-latex
  (dolist (mapping '((emacs-lisp    "common-lisp")
                     (systemverilog "systemverilog") ;; Pygments knows 'systemverilog'
                     (verilog       "verilog")
                     (tcl           "tcl")
                     (sh            "bash")
                     (shell         "bash")
                     (spice         "spice")))  
    (add-to-list 'org-latex-minted-langs mapping)))

;; ------------------------------
;; Export output directory: send all Org exports to build/
;; ------------------------------
(defun my/org-export-to-build-dir (orig-fun extension &optional subtreep pub-dir)
  "Force Org export outputs (.tex, .pdf, etc.) into a build/ subdir
    next to the current Org file, unless PUB-DIR is already specified."
      (let* ((org-file (buffer-file-name))
      (default-build-dir
        (when org-file
          (expand-file-name "build/"
          (file-name-directory org-file)))))
          ;; If caller already provided a pub-dir (e.g. org-publish), respect it.
          (unless pub-dir
          (when default-build-dir
          (unless (file-directory-p default-build-dir)
            (make-directory default-build-dir t))
            (setq pub-dir default-build-dir)))
(apply orig-fun extension subtreep pub-dir nil)))


(advice-add 'org-export-output-file-name :around #'my/org-export-to-build-dir)

;; ------------------------------
;; Validation
;; ------------------------------
 
;; Warn if setup-latex.org is missing
(let ((setup-file setupfile))
  (unless (file-exists-p setup-file)
    (message "⚠️ Org LaTeX setup file not found: %s" setup-file)))

#+END_SRC