...
(defun my/open-pdf-and-note (&optional key-or-pdf)
  ...
  (let* ((is-bib (not (eq key-or-pdf 'generic)))
         ...
         ;; Use robust selector for generic PDFs
          (pdf-raw (if is-bib
             (or (my/citar-extract-first-path (citar-get-files key))
                 (my/select-pdf-project-or-browse))  ; ← fallback added
    ...
    (org-entry-put (point) "NOTER_DOCUMENT" (expand-file-name pdf)))
    (display-buffer (find-file-noselect pdf))  ; display-buffer-alist routes it right
    (org-noter)
    ...
    (let* (;; Book entry at point in *calibredb-search* / *calibredb-show*.
     ...
     (if (re-search-forward "^\\* Overview" nil t)
         (progn
           ...
           (set-window-buffer (split-window-right) (find-file-noselect book-path))
           (org-noter)
