  ...
  (when (derived-mode-p 'pdf-view-mode)
    ...
    (let* ((pdf-file (buffer-file-name))
      ...
      (unless whole-page
        ...
        ;; Handle both old format (just quads) and new fork format (page . quads)
        (setq quads (if (numberp (car-safe raw-edges))
                        (cdr raw-edges)   ; new fork: drop page number
                      raw-edges)))        ; old format
      (if whole-page
          (shell-command
        ...
        (if (and quads
                 (listp quads)
                 (cl-every (lambda (quad)
                             (and (listp quad)
                                  (= (length quad) 4)
                                  (cl-every #'numberp quad)))
                           quads))
            (pdf-view-extract-region-image quads filename)  ; quads first, then filename
