...
(defun my/org-build-id-candidates-from-files (files)
  "Return completion candidates from all annotation IDs in FILES.
Only IDs containing \"annot\" are included."
  (let ((idx (org-id-update-id-locations files 'silent))
        result)
    (cond
     ((hash-table-p idx)
      (maphash
     ...
     ((listp idx)
      ...
      (nreverse result))
     (t
      (user-error "Unexpected org-id index type: %S" (type-of idx))))))
...
(defun my/org-insert-transclude-from-id-with-front-author (&optional id)
  ...
  (let* ((file (read-file-name
                "Annotation Org file: "
                "/wspace/org/" nil t nil
                (lambda (f) (string-match-p "\\.org\\'" f))))
         (files (list file))
         (cands (my/org-build-id-candidates-from-files files))
         (id (or id
                 (if (null cands)
                     (user-error "No IDs found in %s" file)
                   (cdr (assoc (completing-read "Transclude annotation ID: "
                                                cands nil t)
                               cands)))))
         (m  (org-id-find id 'marker)))
    (unless m
      (user-error "ID not found in %s: %s" file id))
...
#+BEGIN_SRC emacs-lisp
;; During many quotes Reduce bloat by skipping drawers
