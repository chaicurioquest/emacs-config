...
;; #+BEGIN_SRC emacs-lisp
;; ;; ──────────────────────────────────────────────────────────────
;; ;; PDF opening after Org → LaTeX export (C-c C-e l o)
;; ;; Also handles [[file:*.pdf]] links in Org files
;; ;; ──────────────────────────────────────────────────────────────
;; (defun my/org-open-pdf-right (file &optional _link)
;;   "Open any PDF/DOCX/PPTX on the right → always clean Org-left + Viewer-right."
;;   (let ((doc-buf (find-file-noselect file)))
;;     (delete-other-windows)               ; ← enforce exactly 2 windows
;;     (split-window-right)                 ; 50/50 (matches org-noter)
;;     (other-window 1)
;;     (switch-to-buffer doc-buf)
;;     (cond
;;      ((derived-mode-p 'pdf-view-mode) (pdf-view-fit-page-to-window))
;;      ((derived-mode-p 'doc-view-mode) (doc-view-fit-page-to-window)))
;;     (other-window -1)                    ; leave cursor in Org file
;;     (message "Viewer on right → %s" (file-name-nondirectory file))))

;; (with-eval-after-load 'org
;;   (add-to-list 'org-file-apps '("\\.pdf\\'" . my/org-open-pdf-right)))
;; #+END_SRC
...
(defun my/load-roam-bib-template (&optional citekey title)
  ...
  (let* ((template (my/load-template "template/roam-bib.org"))
...
;; ── notextn.el: replace my/select-pdf-project-or-browse ──────────────
(defun my/select-pdf-project-or-browse ()
  "Select PDF under notes root (recursive) or browse manually.
Uses `my/notes-root-dir' as the search root — no directory prompt.
Offers a '* Browse anywhere...' escape hatch to pick any file."
  (let* ((root (file-name-as-directory (expand-file-name my/notes-root-dir)))
         ;; Use process-lines to avoid shell quoting issues with spaces
         (pdf-list (ignore-errors
                     (process-lines "find" root
                                    "(" "-type" "f" "-o" "-type" "l" ")"
                                    "-name" "*.pdf")))
         (choice (completing-read
                  (format "Select PDF [root: %s] (or * to browse): "
                          (abbreviate-file-name root))
                  (append (or pdf-list '()) '("* Browse anywhere..."))
                  nil t)))
    (if (string= choice "* Browse anywhere...")
        (read-file-name "Select any file: " root nil t)
      choice)))
#+END_SRC
...
(defun my/open-pdf-and-note (&optional key-or-pdf)
  ...
  (let* ((is-bib (not (eq key-or-pdf 'generic)))
         (key (if is-bib
                ...
                nil))
         (pdf-raw (if is-bib
                      (or (my/citar-extract-first-path (citar-get-files key))
         ...
         (note-file (if is-bib
                      ...
                      (let ((pdf-dir (file-name-directory (expand-file-name pdf))))
                        (read-file-name "Save note as: " pdf-dir nil nil
    ...
    (pop-to-buffer (get-file-buffer note-file))  ; ← CHANGE: notes buf must be current for org-noter
    (org-noter)
    (message "Opened %s note → %s"               ; ← CHANGE: removed duplicate
             (if is-bib "bibliography" "generic")
             (file-name-nondirectory note-file))))
#+END_SRC

** Calibre BibTeX Export

#+BEGIN_SRC emacs-lisp
;; ──────────────────────────────────────────────────────────────
;; Calibre helper functions (laptop only)
;; ──────────────────────────────────────────────────────────────
(when (eq my-device 'laptop)

  (defun calibredb-get-cover (candidate &optional full-path)
    "Get cover image path of CANDIDATE. Handles nil :book-dir."
    (let* ((path (calibredb-getattr candidate :book-dir)))
      (when path
        (let ((cover (expand-file-name "cover.jpg" path)))
          (when (file-exists-p cover)
            (if full-path cover (concat "file:" cover)))))))
  ...
  (defun my/calibredb-export-books-bib ()
    ...
    (let* ((bib-file (expand-file-name books-bib-path))
           (lib-dir  my-calibre-library-dir))
      (let ((exit-code
             (call-process "calibredb" nil "*calibredb-bib*" t
                           "catalog" bib-file
                           "--with-library" lib-dir
                           "--fields" "title,authors,author_sort,publisher,pubdate,isbn,id")))
        (if (not (zerop exit-code))
            (message "calibredb failed; see *calibredb-bib* buffer")
          (let* ((json-str (shell-command-to-string
                            (format "calibredb list --fields=id,formats --with-library '%s' --for-machine"
                                    lib-dir)))
                 (entries (ignore-errors (json-parse-string json-str :array-type 'list))))
            (when entries
              (with-temp-buffer
                (insert-file-contents bib-file)
                (dolist (entry entries)
                  (let* ((id  (number-to-string (gethash "id" entry)))
                         (fmts (gethash "formats" entry))
                         (pdf  (when (listp fmts)
                                 (seq-find (lambda (f) (string-suffix-p ".pdf" f)) fmts))))
                    (when pdf
                      (goto-char (point-min))
                      (when (re-search-forward (format "calibreid = \"%s\"" id) nil t)
  ...
  (with-eval-after-load 'calibredb
    (add-hook 'calibredb-search-mode-hook
              (lambda ()
                (when (and (fboundp 'my/calibredb-export-books-bib)
                           (file-newer-than-file-p
                            (expand-file-name "metadata.db" my-calibre-library-dir)
                            (expand-file-name books-bib-path)))
                  (my/calibredb-export-books-bib)
                  (message "Bib auto-updated (DB newer than bib)"))))))
#+END_SRC
...
with metadata and a NOTER_DOCUMENT property."
    ...
    (unless (derived-mode-p 'calibredb-search-mode 'calibredb-show-mode)
      (user-error "Run this from the calibredb buffer (M-x calibredb first)"))
    (let* (;; Book entry at point in *calibredb-search* / *calibredb-show*.
           ...
           (slug (downcase (replace-regexp-in-string "[^a-z0-9 ]" "" (format "%s - %s" file-author title*))))
     ...
     (if (re-search-forward "^\\* Overview" nil t)
         (progn
           ;; Pre-split: notes=left (current), doc=right — org-noter reuses both
          (display-buffer (find-file-noselect book-path))
          (pop-to-buffer (current-buffer))
          (org-noter)
           (message "Opened: %s by %s with Org-noter session" title* author*))
       (message "Warning: No Overview heading found in %s — Org-noter not started."
                (file-name-nondirectory note-file))))))
...
;; Newest-first image picker — sorts by actual mtime, not filename string.
;; Handles mixed naming (screenshots, timestamps, any filename).
(defun my/read-image-newest-first (dir)
  "Select image from DIR sorted newest first by modification time."
  (let* ((files  (directory-files dir nil "\\.\\(png\\|jpg\\|jpeg\\)\\'" t))
         (sorted (sort files
                       (lambda (a b)
                         (time-less-p
                          (nth 5 (file-attributes (expand-file-name b dir)))
                          (nth 5 (file-attributes (expand-file-name a dir))))))))
    (expand-file-name (completing-read "Select image: " sorted nil t) dir)))
...
#+BEGIN_SRC emacs-lisp
;; ──────────────────────────────────────────────────────────────
;; C-c i s — Capture drag-selected PDF region as timestamped PNG; insert #+CAPTION link in org-noter note.
;; Zero coordinate math. Saves timestamped PNG to images/ sibling of note.
;; ──────────────────────────────────────────────────────────────
(defun my/pdf-screenshot-region-and-save ()
  "Capture screen region with scrot -s, save timestamped PNG, insert org link.
Requires scrot (already installed per config). Draw rectangle over PDF region."
  (interactive)
  (unless (bound-and-true-p org-noter--session)
    (user-error "No org-noter session. Use C-c i a first."))
  (let* ((note-win  (org-noter--get-notes-window))
         (note-buf  (when note-win (window-buffer note-win)))
         (note-file (when note-buf (buffer-file-name note-buf))))
    (unless note-file
      (user-error "No Org note window found."))
    (let* ((image-dir (my/get-image-dir-for-note note-file))
           (timestamp (format-time-string "%Y-%m-%d-%H%M%S"))
           (filename  (expand-file-name (concat timestamp ".png") image-dir)))
      (make-directory image-dir t)
      (call-process-shell-command
       (format "scrot -s %s" (shell-quote-argument filename)))
      (unless (file-exists-p filename)
        (user-error "Screenshot cancelled or scrot failed."))
      (message "Saved → %s  (C-c s → fig to insert)"
...
#+BEGIN_SRC emacs-lisp
;; ──────────────────────────────────────────────────────────────
;; C-c i S — Capture entire current PDF page as timestamped PNG; insert #+CAPTION link in org-noter note.
;; ──────────────────────────────────────────────────────────────
(defun my/pdf-extract-whole-page-and-save ()
  "Render current full PDF page via pdftoppm, save as timestamped PNG, insert org link."
  (interactive)
  ...
  (let* ((note-win  (org-noter--get-notes-window))
    ...
    (unless note-file
      (user-error "No Org note window found. Use C-c i a to open PDF + note."))
    (let* ((page        (pdf-view-current-page))
           (resolution  150)
           (image-dir   (my/get-image-dir-for-note note-file))
           (timestamp   (format-time-string "%Y-%m-%d-%H%M%S"))
           (filename    (expand-file-name (concat timestamp ".png") image-dir))
           (temp-prefix (make-temp-file "pdfcap"))
           (temp-page   (progn
                          (call-process-shell-command
                           (format "pdftoppm -jpeg -r %d -f %d -l %d %s %s"
                                   resolution page page
                                   (shell-quote-argument pdf-file)
                                   (shell-quote-argument temp-prefix)))
                          (car (file-expand-wildcards (concat temp-prefix "*.jpg"))))))
      (make-directory image-dir t)
      (unless temp-page
        (user-error "pdftoppm failed. Ensure poppler-utils is installed."))
      (rename-file temp-page filename t)      ; no crop — direct rename
      (ignore-errors (delete-file temp-prefix))
      (message "Saved → %s  (C-c s → fig to insert)"
         (file-relative-name filename (file-name-directory note-file))))))
#+END_SRC
...
;; ── pdf-view local bindings (all keybindings and advice in one block) ──
...
#+BEGIN_SRC emacs-lisp
;; ── Piper TTS — focused PDF reading (notextn.el) ──
(defvar my/piper-bin   (expand-file-name "~/.local/venv/piper/bin/piper"))
(defvar my/piper-model (expand-file-name "~/.local/share/piper-voices/en_US-ryan-high.onnx"))

(defun my/piper-speak (text)
  "Speak TEXT via Piper TTS, stopping any current speech first."
  (ignore-errors (kill-process (get-process "tts")))
  (let ((tmp (make-temp-file "tts" nil ".txt")))
    (write-region text nil tmp)
    (start-process-shell-command "tts" nil
      (format "cat %s | %s --model %s --output_raw | aplay -r 22050 -f S16_LE -t raw -"
              (shell-quote-argument tmp)
              (shell-quote-argument my/piper-bin)
              (shell-quote-argument my/piper-model)))))

(defun my/speak-pdf-region ()
  "Read selected PDF region aloud."
  (interactive)
  (if (pdf-view-active-region-p)
      (my/piper-speak (mapconcat #'identity (pdf-view-active-region-text) " "))
    (message "No text selected!")))

(defun my/speak-pdf-page ()
  "Read current PDF page aloud."
  (interactive)
  (my/piper-speak (pdf-info-gettext (pdf-view-current-page) '(0 0 1 1))))
