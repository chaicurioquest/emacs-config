...
(with-eval-after-load 'org-noter
  (defun my/org-noter-copy-quoted-text-to-clipboard (orig &rest args)
    "Advice to copy quoted text from org-noter-insert-note to clipboard."
    (let ((start (point))  ; Mark position before insertion
          result)
      (setq result (apply orig args))  ; Run original function
      (when (and (stringp result) (string-match "#\\+BEGIN_QUOTE\\(.*\\)#\\+END_QUOTE" result))
        (let ((quoted-text (match-string 1 result)))
          (kill-new quoted-text)  ; Copy to clipboard
          (message "Copied quoted annotation to clipboard: %s" (truncate-string-to-width quoted-text 50)))))
    result)

  (advice-add 'org-noter-insert-note :around #'my/org-noter-copy-quoted-text-to-clipboard))
#+END_SRC
