...
(with-eval-after-load 'org-noter
  (defun my/org-noter-copy-quoted-text-to-clipboard (orig &rest args)
    "Copy the quoted text (inside #+BEGIN_QUOTE … #+END_QUOTE) to the clipboard after inserting a note."
            ...
            (when (re-search-forward "#\\+BEGIN_QUOTE" nil t)
              (let ((quote-start (point)))
                (when (re-search-forward "#\\+END_QUOTE" nil t)
                  (let ((quoted-text (string-trim (buffer-substring-no-properties quote-start (match-beginning 0)))))
                    (unless (string-empty-p quoted-text)
                      (kill-new quoted-text)
                      (message "Copied quoted annotation to clipboard: %s"
                               (truncate-string-to-width quoted-text 60))))))))))
      result))
