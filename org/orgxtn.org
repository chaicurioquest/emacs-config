...
;; Added: 2026-02-18
;; ========================================
;; Defined at top-level so it exists when org-mode-hook fires,
;; even before org-glossary has loaded.
...
;; Top-level hook — fires on every Org buffer regardless of load order.
...
(use-package org-glossary
  ...
  :after org  ; load after org; no :commands to avoid indefinite deferral
  :config

  ;; Guard var for the recursion-breaking advice below.
  (defvar my/org-glossary--visiting nil)

  ;; Prevent infinite recursion when org-glossary resolves file dependencies.
  ...
  ;; Source files for global term/acronym lookup.
  (setq org-glossary-global-terms
        (mapcar (lambda (f) (expand-file-name f org-roam-directory))
                '("general_acronyms.org" "vlsi_acronyms.org"))
        org-glossary-autodetect-in-headings nil  ; headings excluded from term scan
        org-glossary-print-on-export        nil) ; suppress glossary section on export

  ;; LaTeX export: acronym → \newacronym, first use expands to "long (short)".
  (org-glossary-set-export-spec 'latex 'acronym
    :first-use            "%v (%t)"
    :use                  "%t"
    :definition-structure "\\newacronym{%k}{%t}{%v}")

  ;; LaTeX export: glossary → \newglossaryentry, term only (no expansion).
