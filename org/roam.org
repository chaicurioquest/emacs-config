  ...
  (let ((template-path (expand-file-name filename user-emacs-directory)))
    (if (file-exists-p template-path)
        (with-temp-buffer
          ...
          (goto-char (point-min))
          ;; Point ${setupfile} to <default-directory>/latex/setup-latex.org
          (while (search-forward "${setupfile}" nil t)
            (replace-match
             (expand-file-name "latex/setup-latex.org"
...
(use-package org-roam
  :straight t
  :defer t  ;; Defer for faster startup
  :init
  ...
  ;; Debugging paths (move to config if not needed always)
  ...
  (("C-c r f" . org-roam-node-find)
   ...
   ;; Tag management (comment if not used)
...
#+BEGIN_SRC emacs-lisp
** Avoid race condition in roam DB
...
(use-package citar-org-roam
  ...
  (message "✅ Citar-Org-Roam integration active."))
#+END_SRC
