...
(use-package org-alert
  ...
  ;; ── ADD THIS BLOCK immediately after org-alert-enable ──
  ;; org-alert-check opens all 37 agenda files every interval.
  ;; Kill what it opens, but never touch files already open or modified.
 (advice-add 'org-alert-check :around
            (lambda (orig &rest args)
              (let ((open-before (seq-filter #'buffer-file-name (buffer-list))))
                (apply orig args)
